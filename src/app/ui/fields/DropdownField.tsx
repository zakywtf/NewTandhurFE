...
export default function DropdownField(params: DropdownFieldsProps) {
  const {
    name,
    disabled = false,
    label,
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className={`w-full ${label ? "mt-1.5" : ""}`}>
        <Listbox
          value={ifemptyData(field, ["value"], null)}
          disabled={disabled}
          onChange={(value) => {
        ...
        >
          <div className={`relative`}>
            ...
            >
              <span className="text-tand-appr text-sm font-normal">
                {ifemptyData(field, ["value", "name"], "")}
              </span>
                  ...
                  >
                    <span
                      className={`block truncate ${
                        field && field.value ? "font-medium" : "font-normal"
                      }`}
                    >
                      {item.name}
                    </span>
